(function() {
  ...
  const currencyMethods = {
    ...
    USDT: [
      { name: 'USDT TRC20', icon: 'assets/banks/usdt_trc20.png' },
      { name: 'USDT BEP20', icon: 'assets/banks/usdt_bep20.png' },
  ...
  // Conversion rates to USDT (approx)
  const toUSDT = {
    USDT: 1,
    USD: 1,
    EUR: 1.08,
  ...
  // ----- SIDEBAR LIVE WITHDRAWALS (unchanged) -----
      ...
      <div class="live-withdrawal-row">
        ...
        <div class="live-wd-info">
          ...
          <div class="live-wd-loc">
            <img src="${w.methodIcon}" alt="" style="width:16px;height:16px;vertical-align:middle;border-radius:2px;" onerror="this.style.display='none'"> ${w.methodName}
          </div>
        </div>
        <div>
  ...
  // ----- INLINE WINS & PROOFS CONTENT (FIXED KPI + ALL CURRENCY FILTERS) -----
  function updateHeroAvatarStackInline() {
    ...
    const selected = shuffleArray([...withdrawalPersonas]).slice(0, 3);
    stack.innerHTML = selected.map((p, i) => {
      const style = `width:28px; height:28px; border-radius:50%; border:2px solid #0a1020; margin-left:${i > 0 ? '-8px' : '0'}; position:relative; z-index:${3-i}; background:${p.isFallback ? 'linear-gradient(135deg, #a855f7, #6d28d9)' : 'transparent'}; display:grid; place-items:center; font-size:12px; font-weight:700; color:white; overflow:hidden; flex-shrink:0;`;
      if (p.isFallback) {
        return `<div style="${style}">${p.initials}</div>`;
      }
      return `<div style="${style}"><img src="${p.avatar}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none';this.parentNode.style.background='linear-gradient(135deg, #a855f7, #6d28d9)';this.parentNode.textContent='${p.initials}';"></div>`;
    }).join('');
  }

  // NEW: Generate filter bar with all currencies
  function renderFilterTabsInline() {
    const container = document.querySelector('.liveFilterTabInline').parentNode;
    if (!container) return;
    container.innerHTML = `
      <button class="liveFilterTabInline active" data-filter="all" style="background:rgba(168,85,247,.2); color:#c084fc; border:none; padding:4px 12px; border-radius:999px; font-size:12px; font-weight:600; cursor:pointer;">All</button>
      <button class="liveFilterTabInline" data-filter="completed" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:4px 12px; border-radius:999px; font-size:12px; cursor:pointer;">Completed</button>
      <button class="liveFilterTabInline" data-filter="processing" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:4px 12px; border-radius:999px; font-size:12px; cursor:pointer;">Processing</button>
      <button class="liveFilterTabInline" data-filter="usdt" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:4px 12px; border-radius:999px; font-size:12px; cursor:pointer;">USDT</button>
      <button class="liveFilterTabInline" data-filter="usd" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:4px 12px; border-radius:999px; font-size:12px; cursor:pointer;">USD</button>
      <button class="liveFilterTabInline" data-filter="eur" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:4px 12px; border-radius:999px; font-size:12px; cursor:pointer;">EUR</button>
      <button class="liveFilterTabInline" data-filter="gbp" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:4px 12px; border-radius:999px; font-size:12px; cursor:pointer;">GBP</button>
      <button class="liveFilterTabInline" data-filter="ngn" style="background:rgba(255,255,255,.04); color:#9ca3af; border:1px solid rgba(255,255,255,.1); padding:4px 12px; border-radius:999px; font-size:12px; cursor:pointer;">NGN</button>
    `;
  ...
  // FIXED: KPI totals now in USDT, using full filtered data
  function renderLiveWithdrawalListInline() {
    ...
    // Convert all amounts to USDT for the KPI card
    const totalInUSDT = filtered.reduce((sum, w) => {
      const rate = toUSDT[w.currency] || 1;
      return sum + (w.amount * rate);
    }, 0);
    const totalCount = filtered.length;

    document.getElementById('kpiTotalWithdrawnInline').textContent = formatAmount(totalInUSDT, 'USDT');
    document.getElementById('kpiSuccessfulInline').textContent = totalCount;
    ...
    if (ticker && visible.length > 0) {
      ticker.textContent = `● ${timeAgo(visible[0].time)}`;
    }

    list.innerHTML = visible.map(w => `
      <div style="display:flex; align-items:center; gap:10px; padding:12px 0; border-bottom:1px solid rgba(255,255,255,.04);">
        <div style="width:36px; height:36px; border-radius:50%; 
             background:${w.isFallback ? 'linear-gradient(135deg, #a855f7, #6d28d9)' : 'transparent'}; 
             display:grid; place-items:center; font-size:14px; font-weight:700; color:white;
             overflow:hidden;">
          ${w.isFallback
            ? w.initials
            : `<img src="${w.avatarUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none';this.parentNode.style.background='linear-gradient(135deg, #a855f7, #6d28d9)';this.parentNode.textContent='${w.initials}';">`
          }
        </div>
        <div style="flex:1; min-width:0;">
          ...
          <div style="font-size:11px; color:#9ca3af;">
            <img src="${w.methodIcon}" alt="" style="width:16px;height:16px;vertical-align:middle;border-radius:2px;margin-right:4px;" onerror="this.style.display='none'"> ${w.methodName}
          </div>
        </div>
  ...
  function switchToWinsProofs() {
    ...
    // Reset filter and load list
    inlineActiveFilter = 'all';
    inlineLoadedCount = 6;

    // Regenerate filter tabs (ensures all currencies appear)
  ...
  // Tab click handlers
  document.getElementById('tabGeneralChat')?.addEventListener('click', () => {
    document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
    document.getElementById('tabGeneralChat').classList.add('active');
    document.getElementById('communityChatLayout').style.display = '';
    document.getElementById('winsProofsContent').style.display = 'none';
  });
  document.getElementById('tabWinsProofs')?.addEventListener('click', switchToWinsProofs);
  document.getElementById('tabMarketUpdates')?.addEventListener('click', () => {
    document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
    document.getElementById('tabMarketUpdates').classList.add('active');
    document.getElementById('communityChatLayout').style.display = '';
    document.getElementById('winsProofsContent').style.display = 'none';
  });
  document.getElementById('tabHelpDesk')?.addEventListener('click', () => {
    document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
    document.getElementById('tabHelpDesk').classList.add('active');
    document.getElementById('communityChatLayout').style.display = '';
    document.getElementById('winsProofsContent').style.display = 'none';
  });

  // Quick card – Withdrawals (green icon)
  document.querySelector('.community-quick-ico.green')?.closest('.community-quick-card')?.addEventListener('click', switchToWinsProofs);

  // Inject new live withdrawal (updates sidebar + inline if visible)
