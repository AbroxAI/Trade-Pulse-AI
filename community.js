(function() {
  ...
  function renderWinsProofsHero() {
    const section = document.querySelector('#winsProofsContent .section');
    if (!section) return;
    section.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px; background:linear-gradient(135deg, rgba(168,85,247,.08), rgba(0,0,0,.2)); border-radius:16px; padding:14px;">
        <div style="min-width:0;">
          <span style="background:#ef4444; color:#fff; padding:2px 10px; border-radius:999px; font-size:11px; font-weight:700;">● LIVE</span>
          <div style="font-size:12px; color:#9ca3af; margin-top:8px;">Track live withdrawal activity from verified members.</div>
        </div>
        <div style="display:flex; align-items:center; gap:8px; flex-shrink:0;">
          <div style="display:flex; align-items:center;">
            ${heroAvatars.map((h, i) => `
              <div style="width:36px; height:36px; border-radius:50%; border:2px solid #0a1020; overflow:hidden; background:#1e1b4b; margin-left:${i>0?'-10px':'0'}; z-index:${3-i}; display:grid; place-items:center; font-size:14px; font-weight:700; color:white;">
            ...
            `).join('')}
            <div style="width:36px; height:36px; border-radius:50%; border:2px solid #0a1020; background:linear-gradient(135deg, #a855f7, #6d28d9); margin-left:-10px; z-index:0; display:grid; place-items:center; font-size:12px; font-weight:700; color:white;">
              +128
            </div>
          </div>
          <div>
  ...
  function renderKpiCards() {
    const grid = document.querySelector('#winsProofsContent .kpi-grid');
    if (!grid) return;
    grid.innerHTML = `
  ...
  function renderFilterTabsInline() {
    const container = document.querySelector('#winsProofsContent div[style*="display:flex; gap:6px; margin-bottom:12px"]');
    if (!container) return;
    container.innerHTML = `
  ...
  function avatarHTML(w) {
    ...
    return `
      <div style="width:36px; height:36px; border-radius:50%; background:linear-gradient(135deg, #a855f7, #6d28d9); display:grid; place-items:center; overflow:hidden; position:relative;">
        <img src="${w.avatarUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';">
        <div style="display:none; width:100%; height:100%; align-items:center; justify-content:center; font-size:14px; font-weight:700; color:white;">${initials}</div>
      </div>
    `;
  }
  ...
  function switchToWinsProofs() {
    ...
    // Ensure load more button is attached (replaced after innerHTML)
