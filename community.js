(function() {
  ...
  function updateHeroAvatarStack() {
  ...
  function renderLiveWithdrawalList() {
    const list = document.getElementById('liveWithdrawalList');
    const loadBtn = document.getElementById('loadMoreWithdrawalsBtn');
    if (!list) return;

    let filtered = [...allWithdrawals];
    if (activeFilter === 'completed') filtered = filtered.filter(w => w.status === 'completed');
    else if (activeFilter === 'processing') filtered = filtered.filter(w => w.status === 'processing');
    else if (activeFilter === 'usdt') filtered = filtered.filter(w => w.currency === 'USDT');
    else if (activeFilter === 'ngn') filtered = filtered.filter(w => w.currency === 'NGN');
    ...
    document.getElementById('kpiTotalWithdrawn').textContent = formatAmount(total, displayCurrency);
    document.getElementById('kpiSuccessful').textContent = visible.length;
  ...
  function openLiveWithdrawalsModal() {
    const modal = document.getElementById('liveWithdrawalsModal');
    if (!modal) return;
    updateHeroAvatarStack();
    loadedCount = 6;
    activeFilter = 'all';
    document.querySelectorAll('.liveFilterTab').forEach(t => {
      t.style.background = 'rgba(255,255,255,.04)';
      t.style.color = '#9ca3af';
      t.classList.remove('active');
    });
    const allTab = document.querySelector('.liveFilterTab[data-filter="all"]');
    if (allTab) {
      allTab.style.background = 'rgba(168,85,247,.2)';
      allTab.style.color = '#c084fc';
      allTab.classList.add('active');
    }
    renderLiveWithdrawalList();
    modal.classList.add('open');
  }

  // Expose globally so the onclick in HTML works
  window.openLiveWithdrawalsModal = openLiveWithdrawalsModal;

  document.getElementById('closeLiveWithdrawalsModalBtn')?.addEventListener('click', () => {
    document.getElementById('liveWithdrawalsModal')?.classList.remove('open');
  });
  document.getElementById('liveWithdrawalsModal')?.addEventListener('click', (e) => {
    if (e.target === e.currentTarget) e.currentTarget.classList.remove('open');
  });

  document.querySelectorAll('.liveFilterTab').forEach(tab => {
  ...
  document.getElementById('loadMoreWithdrawalsBtn')?.addEventListener('click', () => {
    loadedCount += 10;
    renderLiveWithdrawalList();
  });

  // Also attach to sidebar card if not using onclick
  document.getElementById('liveWithdrawalSidebarCard')?.addEventListener('click', openLiveWithdrawalsModal);
  ...
  function injectNewLiveWithdrawal() {
    ...
    setTimeout(() => {
      ...
      if (document.getElementById('communityView')?.classList.contains('active')) {
        ...
        if (document.getElementById('liveWithdrawalsModal')?.classList.contains('open')) {
          renderLiveWithdrawalList();
        }
      }
