(function() {
  ...
  // ========== LIVE WITHDRAWALS MODULE (unchanged) ==========
  ...
  // ========== WINS & PROOFS CONTENT (FIXED LAYOUT) ==========
  function buildWinsProofsContent() {
    const container = document.getElementById('winsProofsContent');
    if (!container) return;
    container.innerHTML = `
      <div class="section" style="margin-top:0;">
        <!-- Hero Banner – column layout -->
        <div style="display:flex; flex-direction:column; margin-bottom:16px; background:linear-gradient(135deg, rgba(168,85,247,.08), rgba(0,0,0,.2)); border-radius:16px; padding:14px;">
          <div style="margin-bottom:10px;">
            ...
            <div style="font-size:12px; color:#9ca3af; margin-top:4px;">Track live withdrawal activity from verified members.</div>
          </div>
  ...
  // Fallback avatar with initials (for withdrawal list)
  function avatarHTML(w) {
    const initials = w.initials;
    // Always show initials fallback – no images in withdrawal list
