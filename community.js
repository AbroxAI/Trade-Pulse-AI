(function() {
  ...
  const tabWinsProofs = document.getElementById('tabWinsProofs');
  const tabMarketUpdates = document.getElementById('tabMarketUpdates');
  const tabHelpDesk = document.getElementById('tabHelpDesk');
  const chatLayout = document.getElementById('communityChatLayout');
  const inputBar = document.getElementById('communityInputBar');
  const winsProofsContent = document.getElementById('winsProofsContent');
  const sidebarCol = document.getElementById('communitySidebarCol');

  function setActiveTab(tabElement) {
    document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
    tabElement.classList.add('active');

    if (tabElement === tabWinsProofs) {
      if (chatLayout) chatLayout.style.display = 'none';
      if (inputBar) inputBar.style.display = 'none';
      if (sidebarCol) sidebarCol.style.display = 'none';
      if (winsProofsContent) winsProofsContent.style.display = 'block';
      renderLiveWithdrawalListInline();
    } else {
      if (chatLayout) chatLayout.style.display = 'grid';
      if (inputBar) inputBar.style.display = 'flex';
      if (sidebarCol) sidebarCol.style.display = 'flex';
      if (winsProofsContent) winsProofsContent.style.display = 'none';
    }
  }

  if (tabGeneralChat) tabGeneralChat.addEventListener('click', () => setActiveTab(tabGeneralChat));
  if (tabWinsProofs) tabWinsProofs.addEventListener('click', () => setActiveTab(tabWinsProofs));
  if (tabMarketUpdates) tabMarketUpdates.addEventListener('click', () => setActiveTab(tabMarketUpdates));
  if (tabHelpDesk) tabHelpDesk.addEventListener('click', () => setActiveTab(tabHelpDesk));

  window.switchToWinsProofs = function() {
    if (tabWinsProofs) setActiveTab(tabWinsProofs);
  };

  // ========== LIVE WITHDRAWALS MODULE (INLINE) ==========
  ...
  let loadedCountInline = 6;
  let activeFilterInline = 'all';
  ...
  function updateHeroAvatarStackInline() {
    ...
    stack.innerHTML = selected.map((p, i) => {
      const style = `width:28px; height:28px; border-radius:50%; border:2px solid #0a1020; margin-left:${i > 0 ? '-8px' : '0'}; position:relative; z-index:${3-i}; background:${p.isFallback ? 'linear-gradient(135deg, #a855f7, #6d28d9)' : 'transparent'}; display:grid; place-items:center; font-size:12px; font-weight:700; color:white; overflow:hidden;`;
      if (p.isFallback) {
  ...
  function renderLiveWithdrawalListInline() {
    ...
    if (activeFilterInline === 'completed') filtered = filtered.filter(w => w.status === 'completed');
    else if (activeFilterInline === 'processing') filtered = filtered.filter(w => w.status === 'processing');
    else if (activeFilterInline === 'usdt') filtered = filtered.filter(w => w.currency === 'USDT');
    else if (activeFilterInline === 'ngn') filtered = filtered.filter(w => w.currency === 'NGN');

    const visible = filtered.slice(0, loadedCountInline);

    const total = visible.reduce((sum, w) => sum + w.amount, 0);
    const currenciesSet = new Set(visible.map(w => w.currency));
    const displayCurrency = currenciesSet.size === 1 ? visible[0].currency : 'NGN';
    document.getElementById('kpiTotalWithdrawnInline').textContent = formatAmount(total, displayCurrency);
    document.getElementById('kpiSuccessfulInline').textContent = visible.length;
    ...
    list.innerHTML = visible.map(w => `
      <div style="display:flex; align-items:center; gap:10px; padding:12px 0; border-bottom:1px solid rgba(255,255,255,.04);">
        <div style="width:36px; height:36px; border-radius:50%; 
             ...
             display:grid; place-items:center; font-size:14px; font-weight:700; color:white; overflow:hidden;">
          ${w.isFallback
            ? w.initials
    ...
    if (loadedCountInline >= filtered.length) {
  ...
  // Attach filter events for inline tabs
  document.querySelectorAll('.liveFilterTabInline').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      activeFilterInline = tab.dataset.filter;
      loadedCountInline = 6;
      renderLiveWithdrawalListInline();
    });
  });
  ...
  document.getElementById('loadMoreWithdrawalsBtnInline')?.addEventListener('click', () => {
    loadedCountInline += 10;
    renderLiveWithdrawalListInline();
  });

  // Live injection and updates
  ...
  // Keep times fresh inside the inline list if open
  setInterval(() => {
    if (winsProofsContent && winsProofsContent.style.display === 'block') {
      renderLiveWithdrawalListInline();
    }
  ...
  // ========== FAILSAFE DIRECT CLICK HANDLER ==========
  document.addEventListener('DOMContentLoaded', function() {
    const card = document.getElementById('liveWithdrawalSidebarCard');
    if (card) {
      card.addEventListener('click', function() {
        ...
        if (sidebarCol) sidebarCol.style.display = 'none';
        if (winsProofsContent) winsProofsContent.style.display = 'block';
        // Also update the tab highlight
        document.querySelectorAll('.community-tab').forEach(t => t.classList.remove('active'));
        if (tabWinsProofs) tabWinsProofs.classList.add('active');
        // Refresh the list
        if (typeof renderLiveWithdrawalListInline === 'function') {
          renderLiveWithdrawalListInline();
        }
      });
      console.log('✅ Live Withdrawals sidebar card click handler attached.');
    } else {
      console.warn('⚠️ Could not find #liveWithdrawalSidebarCard – check HTML.');
    }

    // Quick diagnostic: check all critical elements
    const elements = {
      'tabWinsProofs': tabWinsProofs,
      'communityChatLayout': chatLayout,
      'winsProofsContent': winsProofsContent,
      'communitySidebarCol': sidebarCol,
      'communityInputBar': inputBar
    };
    let allGood = true;
    ...
    if (allGood) {
      console.log('🚀 All critical community elements are present.');
    }
  });
  ...
  function startCommunitySimulation() {
    ...
    updateOnlineMembers();
    updateSidebarWithdrawals();
    // Pre-render the inline list and hero avatar stack
