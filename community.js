(function() {
  ...
  // FIXED: KPI totals now use full filtered data, not just loaded slice
  function renderLiveWithdrawalListInline() {
    ...
    // 1. Filter based on active filter
    ...
    // 2. KPIs based on ALL filtered data
    const totalAmount = filtered.reduce((sum, w) => sum + w.amount, 0);
    const totalCount = filtered.length;
    const uniqueCurrencies = [...new Set(filtered.map(w => w.currency))];
    const displayCurrency = uniqueCurrencies.length === 1 ? uniqueCurrencies[0] : 'NGN';

    document.getElementById('kpiTotalWithdrawnInline').textContent = formatAmount(totalAmount, displayCurrency);
    document.getElementById('kpiSuccessfulInline').textContent = totalCount;

    // 3. Visible slice for the list
  ...
  function switchToWinsProofs() {
    ...
    // Hide chat layout and sidebar, show inline content
    ...
    // Reset filters and load inline list
    ...
    const allTabInline = document.querySelector('.liveFilterTabInline[data-filter="all"]');
    if (allTabInline) {
      allTabInline.style.background = 'rgba(168,85,247,.2)';
      allTabInline.style.color = '#c084fc';
      allTabInline.classList.add('active');
    }
    updateHeroAvatarStackInline();
    renderLiveWithdrawalListInline();
  }

  // Expose to global scope so inline onclick can use it
