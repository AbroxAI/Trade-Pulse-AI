(function() {
  ...
  const announcements = [
    {
      id: 1,
      title: "⏳ 43 Spots Left – Closing Soon",
      description: "Only 43 spots remaining for our exclusive VIP Signal Group! Registration closes once filled. Don't miss this opportunity to get premium signals with a 92% win rate. Secure your spot now!",
      date: "Jun 27, 2026 · 8:00 AM",
      image: "assets/announcements/announcement1.jpg",
      reactions: { "👍": 312, "❤️": 176, "🔥": 134, "🚀": 98 }
    },
    {
      id: 2,
  ...
  // Quick cards
  // Chat → General Chat
  document.querySelector('.community-quick-ico.purple')?.closest('.community-quick-card')?.addEventListener('click', switchToGeneralChat);
  ...
  // Withdrawals → Wins & Proofs
  document.querySelector('.community-quick-ico.green')?.closest('.community-quick-card')?.addEventListener('click', switchToWinsProofs);
  // Members (amber) → no action, or you can link to something else
  document.querySelector('.community-quick-ico.amber')?.closest('.community-quick-card')?.addEventListener('click', () => {
    // Optional: could scroll to members section or just stay
  });
