...
(function() {
  ...
  function updateWithdrawEyeIcon() {
    const eye = document.querySelector('.withdraw-eye');
    if (!eye) return;
    if (balanceHidden) {
      eye.innerHTML = '<path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/><path d="M3 21l18-18"/>';
    } else {
      eye.innerHTML = '<path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/>';
    }
  }
  ...
  document.getElementById('copyReferBtn').addEventListener('click', () => {
    const link = 'https://trading-platform.com/ref/JOHN872';
  ...
  document.body.addEventListener('click', e => {
    ...
    const smallAction = e.target.closest('.small-action');
    if(smallAction) {
      const type = smallAction.textContent.trim().toLowerCase();
      if(type.includes('deposit')) openDepositModal();
      else if(type.includes('withdraw')) setView('withdraw');
    }
  });
  ...
  document.getElementById('twoFAModal').addEventListener('click', e => {
    if(e.target === document.getElementById('twoFAModal')) document.getElementById('twoFAModal').classList.remove('open');
  });
  document.getElementById('confirm2FABtn').addEventListener('click', () => {
    const pin = document.getElementById('twoFAPinInput').value;
    const confirmPin = document.getElementById('twoFAConfirmPinInput').value;
    if(pin.length !== 4 || isNaN(pin)) return alert('Please enter a 4‑digit PIN.');
    if(pin !== confirmPin) return alert('PINs do not match.');
    localStorage.setItem('2faPin', pin);
    alert('2‑FA PIN has been set successfully.');
    document.getElementById('twoFAModal').classList.remove('open');
  });
  ...
  if(pushToggle) {
    ...
    pushToggle.addEventListener('change', () => {
      settings.pushNotifications = pushToggle.checked;
    });
  }
