(function() {
  ...
  document.getElementById('homeBalanceEye').addEventListener('click', toggleBalanceHide);
  document.querySelectorAll('.eye').forEach(eye => {
    eye.addEventListener('click', toggleBalanceHide);
  });
  document.getElementById('hideBalanceToggle')?.addEventListener('click', toggleBalanceHide);
  ...
  // Chart – wider viewBox and left padding to prevent label cut‑off
  function buildChart() {
    let maxV = Math.max(...chartData.map(d=>d.value))*1.1||340000;
    let W=400, H=260, pl=52, pr=20, pt=26, pb=30;
