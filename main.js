(function() {
  ...
  // Get logged-in user from sessionStorage (set by auth patch)
  ...
  // Update user in sessionStorage and also in the global users array (for persistence)
  function saveUser(user) {
    ...
    try {
      ...
      localStorage.setItem('tradePulseUser', JSON.stringify(user)); // legacy
    } catch(e) {}
  ...
  // Insufficient balance modal (for withdrawals when balance is zero)
  ...
  function updateDrawerUserInfo() {
    ...
    // Fix copy UID button - make it visible and larger
  ...
  // ----- Zero Balance Withdraw Modal (legacy, but we use the new one) -----
  ...
  // ----- Forgot Password (handled by auth patch, but keep for compatibility) -----
  ...
  // ========== Community Plan Prompt (view‑only + go to invest) ==========
  ...
  // ========== View Switching (MODIFIED: blocks withdraw when balance is zero) ==========
  function setView(v) {
    // NEW: If trying to access withdraw and balance is 0, show insufficient-balance modal and stay where you are
    if (v === 'withdraw' && base.homeBalance <= 0) {
      showInsufficientBalanceModal();
      return;
    }
