(function(){
  ...
  function getStoredUser() { try { return JSON.parse(localStorage.getItem('tradePulseUser') || 'null'); } catch { return null; } }
  ...
  function renderNotificationsModal() {
    ...
    list.innerHTML = notifications.map(n => `
      <div style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid rgba(255,255,255,0.05);opacity:${n.read ? 0.72 : 1};">
  ...
  function updateDrawerUserInfo() {
    ...
    const drawerUserId = $('drawerUserIdValue');
    ...
    if (avatar) {
      ...
      if (user.avatar) {
        ...
        if (img) img.remove();
        if (!initial) { initial = document.createElement('span'); initial.className = 'avatar-initial'; avatar.prepend(initial); }
  ...
  function addTransaction(type, amount, subtitle, meta, currency = null, iconType = null) {
    ...
    if (type === 'Withdrawal') withdrawalsOnly.unshift(tx);
  }
  ...
  function updateAll() {
    ...
    $('planMax4').textContent = base.planMax4 === 500000 ? fmt(base.planMax4) : 'Unlimited';
  ...
  function renderRecentTx() {
    ...
    c.innerHTML = allTransactions.slice(0,4).map(tx => {
  ...
  function renderRecentWithdrawals() {
    ...
    c.innerHTML = withdrawalsOnly.slice(0,4).map(tx => {
  ...
  function renderOverviewCards() {
    ...
    c.innerHTML = cards.map(ca => `<div class="stat"><div class="ico" style="background:${ca.iconBg}"><svg viewBox="0 0 24 24" fill="none" stroke="${ca.iconColor}" stroke-width="2">${ca.icon}</svg></div><div class="title">${ca.title}</div><div class="value">${balanceHidden?'****':fmt(ca.value)}</div><div class="sub" style="color:${ca.subColor}">${ca.sub}</div></div>`).join('');
  ...
  let withdrawRendererTimer = null;
  ...
  function setWithdrawMethod(method) {
    ...
    clearTimeout(withdrawRendererTimer);
    withdrawFormContainer.classList.add('switching');
    withdrawRendererTimer = setTimeout(() => {
  ...
  function renderWithdrawForm(method) {
    ...
    let html = '';
    if (method === 'bank') {
      html = `<div class="field-label">Recipient Account Number</div><div class="amount-input"><input type="text" id="withdrawAccountInput" placeholder="Enter account number" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div><div class="field-label">Bank Name</div><select id="bankSelect" style="width:100%;height:56px;border-radius:16px;border:1px solid rgba(255,255,255,.08);background:#0b1020;color:#fff;padding:0 16px;font-size:16px;margin-bottom:14px;"><option value="GTBank">GTBank</option><option value="Opay">Opay</option><option value="Access Bank">Access Bank</option><option value="First Bank">First Bank</option><option value="UBA">UBA</option><option value="Zenith Bank">Zenith Bank</option></select><div class="field-label">Amount</div><div class="amount-input"><input type="number" id="withdrawInput" placeholder="Enter amount" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"><span class="currency">NGN</span></div><div class="amount-presets"><button type="button" class="preset" data-amount="1000">₦1,000</button><button type="button" class="preset" data-amount="5000">₦5,000</button><button type="button" class="preset" data-amount="10000">₦10,000</button><button type="button" class="preset preset-max" id="maxBtn">Max</button></div><div class="fee-row"><span>Transaction Fee</span><span id="withdrawFee">₦ 50.00</span></div><div class="receive-row"><span>You Will Receive</span><span id="receiveAmount">₦ 0.00</span></div><span class="field-error" id="withdrawError"></span><button class="withdraw-btn" id="requestWithdrawalBtn">Request Withdrawal</button><div class="notice-box"><div class="notice-title">Withdrawal Notice</div><div class="notice-text">Please ensure your details are correct. Incorrect details may cause delay or failed transactions.</div></div>`;
    } else if (method === 'usdt_trc20' || method === 'usdt_bep20') {
      const network = method === 'usdt_trc20' ? 'TRC20' : 'BEP20';
      html = `<div class="field-label">Wallet Address (${network})</div><div class="amount-input"><input type="text" id="withdrawAccountInput" placeholder="Enter ${network} wallet address" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div><div class="field-label">Amount (USDT)</div><div class="amount-input"><input type="number" id="withdrawInput" placeholder="Enter amount" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"><span class="currency">USDT</span></div><div class="amount-presets"><button type="button" class="preset" data-amount="10">10 USDT</button><button type="button" class="preset" data-amount="50">50 USDT</button><button type="button" class="preset" data-amount="100">100 USDT</button><button type="button" class="preset" data-amount="500">500 USDT</button><button type="button" class="preset preset-max" id="maxBtn">Max</button></div><div class="fee-row"><span>Network Fee</span><span id="withdrawFee">1 USDT</span></div><div class="receive-row"><span>You Will Receive</span><span id="receiveAmount">0.00 USDT</span></div><span class="field-error" id="withdrawError"></span><button class="withdraw-btn" id="requestWithdrawalBtn">Request Withdrawal</button><div class="notice-box"><div class="notice-title">Withdrawal Notice:</div><div class="notice-text">Ensure the wallet address is correct. USDT withdrawals are processed automatically.</div></div>`;
    } else if (method === 'ewallet') {
      html = `<div class="field-label">E-Wallet ID / Email</div><div class="amount-input"><input type="text" id="withdrawAccountInput" placeholder="Enter e-wallet ID or email" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"></div><div class="field-label">Amount</div><div class="amount-input"><input type="number" id="withdrawInput" placeholder="Enter amount" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"><span class="currency">NGN</span></div><div class="amount-presets"><button type="button" class="preset" data-amount="1000">₦1,000</button><button type="button" class="preset" data-amount="5000">₦5,000</button><button type="button" class="preset" data-amount="10000">₦10,000</button><button type="button" class="preset" data-amount="25000">₦25,000</button><button type="button" class="preset preset-max" id="maxBtn">Max</button></div><div class="fee-row"><span>Transaction Fee</span><span id="withdrawFee">₦ 50.00</span></div><div class="receive-row"><span>You Will Receive</span><span id="receiveAmount">₦ 0.00</span></div><span class="field-error" id="withdrawError"></span><button class="withdraw-btn" id="requestWithdrawalBtn">Request Withdrawal</button><div class="notice-box"><div class="notice-title">Withdrawal Notice:</div><div class="notice-text">Ensure your e-wallet details are correct. Instant processing.</div></div>`;
    }
  ...
  function bindWithdrawFormEvents() {
    ...
    if (requestBtn) {
      requestBtn.addEventListener('click', () => {
        ...
        $('confirmBank').textContent = activeWithdrawMethod === 'bank' ? `${bankName} - ${accountNumber}` : `${bankName}`;
  ...
  $('confirmResetPasswordBtn')?.addEventListener('click', () => {
    ...
    if (!username || !inviteCode || !newPassword || !confirmPassword) { errorBox.textContent = 'Please fill in all fields.'; errorBox.style.display = 'block'; return; }
    if (inviteCode !== VALID_INVITE_CODE) { errorBox.textContent = 'Invalid invitation code.'; errorBox.style.display = 'block'; return; }
    if (newPassword !== confirmPassword) { errorBox.textContent = 'Passwords do not match.'; errorBox.style.display = 'block'; return; }
    const user = getStoredUser();
    if (!user || user.username !== username) { errorBox.textContent = 'No matching account found.'; errorBox.style.display = 'block'; return; }
  ...
  function setCommunityViewOnlyPrompt() { showSuccess('Community Chat', 'Community chat is view-only right now. Select an investment plan first, then come back to chat.'); }
  ...
  function setView(v) {
    ...
    $('pageTitle').textContent = { home:'Home', invest:'Invest', withdraw:'Withdraw', wallet:'Wallet', community:'Community' }[v] || 'Home';
  ...
  $('upgradeVipBtn')?.addEventListener('click', () => { setView('invest'); setTimeout(() => $('vipPlanCard')?.scrollIntoView({ behavior:'smooth' }), 200); });
  $('promoUpgradeBtn')?.addEventListener('click', () => { setView('invest'); setTimeout(() => $('vipPlanCard')?.scrollIntoView({ behavior:'smooth' }), 200); });
  ...
  $('copyReferBtn')?.addEventListener('click', () => { if (currentUser && currentUser.referralCode) navigator.clipboard.writeText(currentUser.referralCode).then(() => addNotification('Copied!', 'Referral code copied to clipboard', 'success')); });
  ...
  $('drawerLogoutBtn')?.addEventListener('click', () => openModal('logoutConfirmModal'));
  $('confirmLogoutBtn')?.addEventListener('click', () => {
    sessionStorage.clear();
    localStorage.removeItem('tradePulseUser');
  ...
  $('closeSuccessModalBtn')?.addEventListener('click', () => closeModal('withdrawalSuccessModal'));
  ...
  $('closeSuccessModalBtn2')?.addEventListener('click', () => closeModal('successModal'));
  ...
  if (savedUser) {
    ...
    base.totalProfit = savedUser.totalProfit || 0;
