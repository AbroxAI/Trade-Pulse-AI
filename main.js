(function() {
  ...
  // ----- Forgot Password (handled by auth patch) -----
  ...
  $('confirmResetPasswordBtn')?.addEventListener('click', () => {
    // Already handled by auth script
  });
  ...
  // ========== Wallet Action Buttons (Transfer, History, Cards) ==========
  // Transfer (4th wallet-action button)
  const walletTransferBtn = document.querySelector('.wallet-actions .wallet-action:nth-child(4)');
  if (walletTransferBtn) {
    walletTransferBtn.addEventListener('click', () => openModal('transferModal'));
  }
  // History (5th wallet-action button)
  const walletHistoryBtn = document.querySelector('.wallet-actions .wallet-action:nth-child(5)');
  if (walletHistoryBtn) {
    walletHistoryBtn.addEventListener('click', () => showModalWithGuard('All Transactions', allTransactions));
  }
  // Cards (6th wallet-action button)
  const walletCardsBtn = document.querySelector('.wallet-actions .wallet-action:nth-child(6)');
  if (walletCardsBtn) {
    walletCardsBtn.addEventListener('click', () => openModal('cardModal'));
  }
  // Withdraw (2nd wallet-action button) - just navigate to withdraw page
  const walletWithdrawBtn = document.querySelector('.wallet-actions .wallet-action:nth-child(2)');
  if (walletWithdrawBtn) {
    walletWithdrawBtn.addEventListener('click', () => setView('withdraw'));
  }
