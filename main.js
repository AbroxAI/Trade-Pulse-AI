(function() {
  ...
  function getStoredUser() {
    try { 
      return JSON.parse(sessionStorage.getItem('tradePulseCurrentUser') || 'null');
    } catch { return null; }
  }
  ...
  // Payment not detected modal (for deposit & investment)
  ...
  // Insufficient balance modal (for withdrawal request when balance is zero)
  ...
  let base = {
    ...
    txFee: FEE,
    activePlans: 0,
    planMin1: 10000,    // ★ Beginner minimum ₦10,000
    planMax1: 49999,
    planMin2: 50000,
    planMax2: 199999,
    planMin3: 200000,
  ...
  function updateNotificationBadge() {
    ...
    // No drawer badge – only topbar badge
    if (topBadge) {
  ...
  function updateDrawerUserInfo() {
    ...
    if (drawerFullName) drawerFullName.textContent = user.username || user.fullName || 'User';
    ...
    if (copyBtn && user.userId) {
      ...
      newBtn.addEventListener('click', (e) => {
        ...
        navigator.clipboard.writeText(user.userId).then(() => {
          // no notification – just visual feedback on button
  ...
  function renderWithdrawForm(method) {
    ...
    if (method === 'bank') {
      html = `
        ...
        <div class="amount-presets"><button type="button" class="preset" data-amount="1000">₦1k</button><button type="button" class="preset" data-amount="5000">₦5k</button><button type="button" class="preset" data-amount="10000">₦10k</button><button type="button" class="preset preset-max" id="maxBtn">Max</button></div>
      ...
      html = `
        ...
        <div class="field-label">Amount</div>
        <div class="amount-input"><input type="number" id="withdrawInput" placeholder="Enter amount" style="background:transparent;border:none;color:#fff;font-size:16px;width:100%;outline:none"><span class="currency">NGN</span></div>
        <div class="amount-presets"><button type="button" class="preset" data-amount="1000">₦1k</button><button type="button" class="preset" data-amount="5000">₦5k</button><button type="button" class="preset" data-amount="10000">₦10k</button><button type="button" class="preset" data-amount="25000">₦25k</button><button type="button" class="preset preset-max" id="maxBtn">Max</button></div>
        <div class="fee-row"><span>Transaction Fee</span><span id="withdrawFee">₦ 50.00</span></div>
        <div class="receive-row"><span>You Will Receive</span><span id="receiveAmount">₦ 0.00</span></div>
        <span class="field-error" id="withdrawError"></span>
        <button class="withdraw-btn" id="requestWithdrawalBtn">Request Withdrawal</button>
  ...
  // ----- Zero Balance Withdraw Modal (legacy) -----
  ...
  // ========== DEPOSIT FLOW (minimum ₦10,000 enforced) ==========
  ...
  $('confirmDepositBtn')?.addEventListener('click', () => {
    ...
    if (isNaN(amount) || amount <= 0) {
      err.textContent = 'Enter a valid deposit amount'; err.style.display = 'block';
      setTimeout(() => err.style.display = 'none', 3000);
      return;
    }
    if (amount < 10000) {
      err.textContent = 'Minimum deposit is ₦10,000'; err.style.display = 'block';
      setTimeout(() => err.style.display = 'none', 3000);
      return;
    }
  ...
  // ========== INVESTMENT FLOW (minimum ₦10,000 enforced, red error) ==========
  function openInvestModal(planName, min, planKey) {
    $('investPlanName').textContent = planName;
    $('investMin').textContent = min.toLocaleString();
    const rateInfo = planRates[planKey];
    if (rateInfo) {
      $('investDailyRate').textContent = rateInfo.range;
      currentPlanAvg = rateInfo.avg;
    }
  ...
  function updateInvestExpected() {
    ...
    const totalReturn = amount + (amount * currentPlanAvg / 100 * 30);
  ...
  $('confirmInvestBtn')?.addEventListener('click', () => {
    ...
    if (isNaN(amount) || amount <= 0) {
      if (investErr) { investErr.textContent = 'Enter a valid amount.'; investErr.style.display = 'block'; setTimeout(() => investErr.style.display = 'none', 3000); }
      return;
    }
    if (amount < 10000) {
      if (investErr) { investErr.textContent = 'Minimum investment is ₦10,000'; investErr.style.display = 'block'; setTimeout(() => investErr.style.display = 'none', 3000); }
      return;
    }
  ...
  document.querySelectorAll('.invest-now-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const min = plan === 'beginner' ? 10000 : plan === 'standard' ? 50000 : plan === 'premium' ? 200000 : 500000;
      openInvestModal(card.querySelector('.plan-title').textContent, min, plan);
    });
  });

  // ========== SHARED PAYMENT MODAL HANDLER ==========
  ...
  // ========== Community Plan Prompt (only on send) ==========
  ...
  if (communitySendBtn) {
    communitySendBtn.addEventListener('click', () => {
      setCommunityPlanPrompt();
    });
  }
  if (communityInputField) {
    communityInputField.addEventListener('keypress', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        setCommunityPlanPrompt();
      }
  ...
  if (walletActions[0]) walletActions[0].addEventListener('click', () => openModal('depositModal'));
  ...
  // Copy Referral Code – no notification badge
  ...
  // ========== Modal overlay click to close (all modals) ==========
